...
class Joint(object):
    ...
    def __init__(self, name=None, offset=Transform3d(), joint_type='fixed', axis=(0.0, 0.0, 1.0), jrange=None,
    ...
    def __repr__(self):
        return "Joint(name='{0}', offset={1}, joint_type='{2}', axis={3}, range={4})".format(self.name,
                                                                                  self.offset,
                                                                                  self.joint_type,
                                                                                  self.axis,
                                                                                  self.range)
...
class Frame(object):
    ...
    def get_transform(self, theta):
        ...
        if self.joint.joint_type == 'revolute':
            t = Transform3d(rot=axis_angle_to_quaternion(theta * self.joint.axis), dtype=dtype, device=d)
        elif self.joint.joint_type == 'prismatic':
            t = Transform3d(pos=theta * self.joint.axis, dtype=dtype, device=d)
        elif self.joint.joint_type == 'fixed':
            t = Transform3d(default_batch_size=theta.shape[0], dtype=dtype, device=d)
        else:
